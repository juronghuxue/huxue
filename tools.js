...
define(function(require, exports, module) {
    ...
    /**
     * 倒计时
     * @param {Number} timeStamp 倒计时时间戳(s)
     * @param {Function} secondFn(hh,mm,ss) 读秒回调函数，提供时(hh)、分(mm)、秒(ss)字符串
     * @param {Function} finishFn 倒计时时间到回调函数
     */
    function countDown(timeStamp, secondFn, finishFn) {
        if (typeof timeStamp == "undefined" || typeof timeStamp != "number") {
            return;
        }
        var _secondFn = secondFn || function() {},
            _finishFn = finishFn || function() {},
            _timer, _timeStamp = timeStamp,
        ...
        function count() {
            ...
            if (_timeStamp <= 0) {
                ...
                _secondFn(_hour, _minute, _second);
                _finishFn();
            }
            _hour = parseInt(_timeStamp / 3600);
            _minute = parseInt(_timeStamp / 60) % 60;
