...
define(function(require, exports, modules) {
	...
	// 弹窗内生成支付列表
	function createPayHtml(opt) {
		t.http({
			url: '/hr/api/pay/pay_html',
			type: "POST",
			data: {
				WIDout_trade_no: opt.orderid
			},
			success: function(res) {
				if (res.result == 0) {
					$(opt.node).html(res.data)
				} else {
					t.tips(opt.node, 2, res.msg)
				}
			},
		});
	}
	...
	exports.createPayHtml = createPayHtml
